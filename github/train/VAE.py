...
# ------------------------------------------
# Transformer VAE 模型 + 相对位置偏置 + 所有自定义模块（完整融合）
# ------------------------------------------

class RelativePositionBias(nn.Module):
    def __init__(self, num_buckets=64, base_max_distance=256, n_heads=8):
        super().__init__()
        self.num_buckets = num_buckets
        self.base_max_distance = base_max_distance  # 作为默认上限
    ...
    def forward(self, relative_position):
        ...
        if L <= 128:
            ...
            max_distance = 512  # 你也可以改成 min(1024, L)

        rp_bucket = self._relative_position_bucket(relative_position, self.num_buckets, max_distance)
...
class MultiHeadAttention(nn.Module):
    ...
    def transpose_scores(self, x):
        ...
        return x.view(*new_shape).transpose(-3, -2)
...
class TransformerVAE(nn.Module):
    def __init__(self, input_dim=640, hidden_dim=256, num_heads=8, num_layers=4, dropout=0.1):
        super().__init__()
        self.input_fc = nn.Linear(input_dim, hidden_dim)
        self.input_gate = nn.Linear(hidden_dim, hidden_dim)  # 改为 hidden_dim 作为输入
    ...
    def gated_fusion(self, x, residual):
        gate = torch.sigmoid(self.input_gate(x))  # 修改：x -> x_proj
    ...
    def forward(self, x, attention_mask=None):
        ...
        x_activated = self.input_activation(x_proj)
        x_normed = self.input_norm(x_activated)
        x = self.gated_fusion(x_normed, x_proj)  # 输入两个是同一维度 (B, L, hidden_dim)

        z_enc = self.encoder(x, bias)
        mu = self.fc_mu(z_enc)
        logvar = self.fc_logvar(z_enc)
        z_sample = self.reparameterize(mu, logvar)
        z_dec = self.decoder(z_sample, bias)

        q = self.q_proj(z_dec)
        k = self.k_proj(z_dec)
        score = torch.matmul(q, k.transpose(-1, -2)) / (q.shape[-1] ** 0.5)
        px = torch.stack([-score, score], dim=-1)
        return px, mu, logvar



    def kl_loss(self, mu, logvar, attention_mask):
        mask = ~attention_mask
        mu = mu[mask]
        logvar = logvar[mask]
        if torch.isnan(mu).any() or torch.isnan(logvar).any():
            for i in range(mu.shape[0]):
                if torch.isnan(mu[i, :]).any():
                    print(f'mu[{i}, :3]: {mu[i, :3]}')
                elif torch.isnan(logvar[i, :]).any():
                    print(f'logvar[{i}, :3]: {logvar[i, :3]}')
            raise ValueError("mu 或 logvar 中包含 nan 值")
