...
# from TVAE.UFold.process_data_newdataset import one_hot
# 加载模型
from VAEone import TransformerVAE
...
if torch.cuda.is_available():
    ...
    print("Using GPU2:", torch.cuda.get_device_name(0))
else:
...
# embs_path = '/home/meixy23/TVAE/NEW/output_emb.pkl'
# with open(embs_path, 'rb') as file:
#     RNA_emb_dict = pickle.load(file)
# === 读取 RNA 序列字符串的 pkl 文件 ===
seqs_path = '/home/meixy23/TVAE/Rf1_RNA_emb_dict.pkl'
with open(seqs_path, 'rb') as file:
    RNA_seq_dict = pickle.load(file)
...
#################################################################################
# ACGU → one-hot 编码
def rna_seq_to_onehot(seq):
    """
    将 RNA 序列字符串转换为 one-hot 编码张量，形状为 [L, 4]
    """
...
def Cal_eval_score(model, dataset, Name):
    ...
    for idx, (RNA_emb, label) in enumerate(dataset):
        ...
        prob = torch.softmax(out[0, :, :], dim=-1)[:, :, 1]
        # print(f"[DEBUG] 预测概率均值: {prob.mean().item():.4f}, 最大值: {prob.max().item():.4f}")

        # 得到标签和配对矩阵的 numpy 形式
        n_data = (torch.softmax(out[0, :, :], dim=-1)[:, :, 1] > 0.6).to(torch.int16).detach().cpu().numpy()
        pair_mat = label.detach().cpu().numpy()  # 确保 label 是 tensor

        if idx < 5:
            plt.imshow(n_data)
            plt.colorbar()
            plt.savefig(f'./Fig6/{str(idx).zfill(2)}{Name}_预测.png')
            plt.close()
            plt.imshow(pair_mat)
            plt.colorbar()
            plt.savefig(f'./Fig6/{str(idx).zfill(2)}{Name}_真实.png')
            plt.close()
...
for key in RNA_pair_labels_dict.keys():
    ...
    if key_upper not in RNA_seq_dict:
        print(f"❌ 找不到序列：{key_upper}")
        continue
    ...
    RNA_seq = key_upper
    Emb = rna_seq_to_onehot(RNA_seq)  # ✅ 将序列转换为 one-hot
